...
class Operations:
    ...
    # 轮询等待
    def wait(self, fun, by, value, timeout, period):
        now = time.time()
        end = now + timeout
        while time.time() <= end:
            try:
                ...
                print(f"定位元素失败，等待{period}秒后重试")
                time.sleep(period)
        # 轮询等待失败后返回报错
        return fun(by, value)

    def find_element(self, locator, timeout=60, period=5):
        ...
        if locator_text == "css":
            ...
            return self.wait(self.driver.find_element, By.CSS_SELECTOR, locator_value, timeout, period)
        if locator_text == "id":
            # return self.driver.find_element(By.ID, locator_value)
            return self.wait(self.driver.find_element, By.ID, locator_value, timeout, period)
        if locator_text == "name":
            # return self.driver.find_element(By.NAME, locator_value)
            return self.wait(self.driver.find_element, By.NAME, locator_value, timeout, period)
        if locator_text == "link":
            # return self.driver.find_element(By.LINK_TEXT, locator_value)
            return self.wait(self.driver.find_element, By.LINK_TEXT, locator_value, timeout, period)
        if locator_text == "partial_link":
            # return self.driver.find_element(By.PARTIAL_LINK_TEXT, locator_value)
            return self.wait(self.driver.find_element, By.PARTIAL_LINK_TEXT, locator_value, timeout, period)
        if locator_text == "tag":
            # return self.driver.find_element(By.TAG_NAME, locator_value)
            return self.wait(self.driver.find_element, By.TAG_NAME, locator_value, timeout, period)
        if locator_text == "class":
            # return self.driver.find_element(By.CLASS_NAME, locator_value)
            return self.wait(self.driver.find_element, By.CLASS_NAME, locator_value, timeout, period)
        if locator_text == "xpath":
            # return self.driver.find_element(By.XPATH, locator_value)
            return self.wait(self.driver.find_element, By.XPATH, locator_value, timeout, period)
        # TODO: 可以继续封装更多的自定义定位方式。

    # 封装过的定位多个元素
    def find_elements(self, locator, timeout=60, period=5):
        locator_text = locator.split(":")[0]
        locator_value = ":".join(locator.split(":")[1:])
        logger.info(f"定位元素,使用定位语句={locator}")
        if locator_text == "css":
            # return self.driver.find_elements(By.CSS_SELECTOR, locator_value)
            return self.wait(self.driver.find_elements, By.CSS_SELECTOR, locator_value, timeout, period)
        if locator_text == "id":
            # return self.driver.find_elements(By.ID, locator_value)
            return self.wait(self.driver.find_elements, By.ID, locator_value, timeout, period)
        if locator_text == "name":
            # return self.driver.find_elements(By.NAME, locator_value)
            return self.wait(self.driver.find_elements, By.NAME, locator_value, timeout, period)
        if locator_text == "link":
            # return self.driver.find_elements(By.LINK_TEXT, locator_value)
            return self.wait(self.driver.find_elements, By.LINK_TEXT, locator_value, timeout, period)
        if locator_text == "partial_link":
            # return self.driver.find_elements(By.PARTIAL_LINK_TEXT, locator_value)
            return self.wait(self.driver.find_elements, By.PARTIAL_LINK_TEXT, locator_value, timeout, period)
        if locator_text == "tag":
            # return self.driver.find_elements(By.TAG_NAME, locator_value)
            return self.wait(self.driver.find_elements, By.TAG_NAME, locator_value, timeout, period)
        if locator_text == "class":
            # return self.driver.find_elements(By.CLASS_NAME, locator_value)
            return self.wait(self.driver.find_elements, By.CLASS_NAME, locator_value, timeout, period)
        if locator_text == "xpath":
            # return self.driver.find_elements(By.XPATH, locator_value)
            return self.wait(self.driver.find_elements, By.XPATH, locator_value, timeout, period)
        # TODO: 可以继续封装更多的自定义定位方式。
